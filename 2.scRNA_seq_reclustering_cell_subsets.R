...
eso.merge.B$celltype_sub[eso.merge.B$celltype_top == "B cells" & eso.merge.BPlasma$RNA_snn_res.1 %in% c(0,8)] = "Activated B"
...
eso.merge.B <- RunFastMNN(object.list = SplitObject(eso.merge.BPlasma, split.by = 'dataset'))
...
p4 <- DimPlot(eso.merge.B, reduction = 'umap',  ncol=3, pt.size=0.1, 
              label = T,repel = TRUE)

ggplot2::ggsave("BPlasma_umap_celltype_sub.pdf", p4, width=6 ,height=4)
...
library(patchwork)
library(monocle)
exprs <- as(as.matrix(sub@assays$RNA@counts), 'sparseMatrix')
...
phenoData <- sub@meta.data
phenoData$celltype <- episub@active.ident
...
cds <- reduceDimension(cds, max_components = 2,
                       method = 'DDRTree')
cds <- orderCells(cds, reverse=TRUE)  ##reverse only for last time ("PEC-A2","DSS1-driven cell")


p3 = plot_cell_trajectory(cds,color_by="Pseudotime", cell_size=1,show_backbone=TRUE)
ggplot2::ggsave("BPlasma_clusters_monocle2_time.pdf", p3, width=6,height=6)
p3 = plot_cell_trajectory(cds,color_by="celltype_sub", cell_size=1,show_backbone=TRUE)
ggplot2::ggsave("BPlasma_clusters_monocle2.pdf", p3, width=6,height=6)



#加载R包#首先你需要把这些包安装好  slingshot
library(slingshot)
...
umap=eso.merge.BPlasma@reductions$umap@cell.embeddings
